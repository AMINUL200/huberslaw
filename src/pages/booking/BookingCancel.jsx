...
const BookingCancel = () => {
  ...
  const handleReschedule = async () => {
    try {
      ...
      // Here you would typically call an API to handle rescheduling
      // For now, we'll navigate to the booking page or show a message
      toast.info('Redirecting to rescheduling page...');
      setTimeout(() => {
        navigate('/book-appointment'); // Adjust this route as needed
      }, 1000);
    } catch (err) {
      ...
      <div className="container mx-auto px-4 py-8 -mt-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            {/* Next Steps Card */}
            <div className="bg-white rounded-2xl shadow-xl border border-[#E8EEF4] p-6">
              <h3 className="text-xl font-bold text-[#0A1A2F] mb-4">What's Next?</h3>
              <div className="space-y-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <div className="font-semibold text-[#0A1A2F]">Cancellation Confirmed</div>
                    <p className="text-sm text-gray-600">Your appointment has been removed from our schedule.</p>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <div className="font-semibold text-[#0A1A2F]">No Further Action</div>
                    <p className="text-sm text-gray-600">No payment or additional steps are required.</p>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <div className="font-semibold text-[#0A1A2F]">Future Bookings</div>
                    <p className="text-sm text-gray-600">You're welcome to book again anytime.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <button
                ...
                className={`w-full bg-[#CBA054] text-white py-3 px-6 rounded-lg font-semibold transition-all duration-300 flex items-center justify-center space-x-2 ${
                  rescheduleLoading 
                    ? 'opacity-50 cursor-not-allowed' 
                    : 'hover:bg-[#B89246] hover:scale-105'
                }`}
              >
                {rescheduleLoading ? (
                ...
                <span>
                  {rescheduleLoading ? 'Processing...' : 'Reschedule Appointment'}
                </span>
              </button>

              <Link
                to="/"
                className="w-full bg-[#0A1A2F] text-white py-3 px-6 rounded-lg font-semibold hover:bg-[#1E354F] transition-all duration-300 flex items-center justify-center space-x-2"
              >
                <Home className="w-5 h-5" />
              ...
              <Link
                to="/contact"
                className="w-full border-2 border-[#0A1A2F] text-[#0A1A2F] py-3 px-6 rounded-lg font-semibold hover:bg-[#0A1A2F] hover:text-white transition-all duration-300 text-center block"
              >
                Contact Support
              </Link>
            </div>
            ...
            {/* Support Information */}
            <div className="bg-[#F4EEDC] rounded-2xl p-4 text-center">
              <p className="text-sm text-[#0A1A2F]">
                Need help?{' '}
                <Link to="/contact" className="text-[#CBA054] font-semibold hover:underline">
